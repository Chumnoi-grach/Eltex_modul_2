...
static void handle_balance_tree(PhoneBook *pb);
static void handle_display_tree(PhoneBook *pb);
...
void run_phonebook(PhoneBook *pb) {
    ...
    while (1) {
        ...
        switch (choice) {
            ...
            case 7: handle_balance_tree(pb); break;
            case 8: handle_display_tree(pb); break;
...
static void display_menu(void) {
    ...
    printf("7. Балансировка древа\n");
    printf("8. Вывод древа\n");
    printf("0. Выход\n");
}
...
static void handle_add_contact(PhoneBook *pb) {
    ...
    if (strlen(c.patronymic) > 0 && (!validate_name(c.patronymic))) {
        printf("Ошибка: отчество не соответствуют правильному формату\n");
        return;
    }
    if (strlen(c.phone) > 0 && !validate_phone(c.phone)) {
        printf("Ошибка: номер телефона не соответствует правильному формату\n");
        return;
    }
    if (strlen(c.email) > 0 && !validate_email(c.email)) {
        printf("Ошибка: email не соответствует правильному формату\n");
        return;
    }
...
static void handle_delete_contact(PhoneBook *pb) {
    ...
    if (id < 1) printf("Ошибка, отрицательный ID\n");

    printf("Удалить контакт ID %d? (y/n): ", id);
    char confirm = get_char();
    if (confirm == 'y' || confirm == 'Y') {
        res = delete_contact(pb, id);
        if (res == 0) {
...
static void handle_balance_tree(PhoneBook *pb) {
    printf("Принудительная сортировка дерева...\n");
    balance_tree(pb);
    printf("Дерево сбалансировано...\n");
}

static void handle_display_tree(PhoneBook *pb) {
    display_tree_hierarchy(pb);
}